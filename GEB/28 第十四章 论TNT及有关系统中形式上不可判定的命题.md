---
title: 第十四章 论TNT及有关系统中形式上不可判定的命题
volume: 下篇：异集璧EGB
...
$\exists$a:WJU-PROOF-PAIR{a, SSSSSSSSSSSSSSSSSSSSSSSSSSSSSS0/a}
...
下面的对照表中，左边一列给出这种运算的一些例子，而右边一列则显示其哥德尔数的对应变化
  ...
  ～$\exists$a:$\exists$a:a\"=(SSa·SSa)     , 333, 262, 636, 333, 262, 163, 636, 262, 163, 163, 111, 362, 123, 123, 262, 236, 123, 123, 262, 163, 323

  全部自由变元都换成4的数字表示             $\downarrow$

  ～$\exists$a:$\exists$a:SSSS0=(SSa·SSa)   , 333, 262, 636, 333, 262, 163, 636, 123, 123, 123, 123, 666, 111, 362, 123, 123, 262, 236, 123, 123, 262, 163, 323
  ----------------------------------------- ---------------------------------------------------------------------------------------------------------------------
...
\~$\exists$a:$\exists$a:＜TNT-PROOF-PAIR{a, a}$\wedge$ARITHMOQUINE{a\", a}＞
...
\~$\exists$a:$\exists$a:＜TORTOISE-PAIR{a, a}$\wedge$TENTH-POWER{SS0/a\", a}＞
...
我们要停下来喘口气，并回顾一下我们做了些什么。就我所知，要想比较透彻地把握住这种代入方法，最好的办法是直截了当地把它与蒯恩形式的说谎者悖论作一个比较。它们之间的对应是
  ...
  把一个谓语自身作为引语放在该谓语之前（"㧟摁化"）                             $\Leftrightarrow$   把一符号串的哥德尔数代入一开公式（"算术㧟摁化"）

  摁时得到假句子（没有主语。仅有一个谓语）                                     $\Leftrightarrow$   G的"服"号串（TNT的一个开公式）

  "㧟摁时得到假句子"（上面的那个谓语，加上引号）                               $\Leftrightarrow$   数u（上述开公式的哥德尔数）

  "㧟摁时得到假句子"㧟摁时得到假句子（㧟摁上面那个谓语而形成的一个完整句子）   $\Leftrightarrow$   G本身（把u代入"服"号串所形成的句子，即"服"号串的算术㧟摁）
  ---------------------------------------------------------------------------- ------------------- ------------------------------------------------------------
...
![image](images/Formula11.png)

\~$\exists$a:＜TNT-PROOF-PAIR{S0/a, a}$\wedge$ARITHMOQUINE{SSS......SSS0/a\", a}＞

\~$\exists$a:＜TNT-PROOF-PAIR{SS0/a, a}$\wedge$ARITHMOQUINE{SSS......SSS0/a\", a}＞

\~$\exists$a:＜TNT-PROOF-PAIR{SSS0/a, a}$\wedge$ARITHMOQUINE{SSS......SSS0/a\", a}＞
...
5p[^2^](#fn:Chapter14-2)+17q[^17^](#fn:Chapter14-17)-177=0
