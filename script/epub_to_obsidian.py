...
def extract_footnote_ref_from_href(href: str) -> str | None:
    ...
    if not href:
        return None
    # duokan pattern: #B_1, #A_1 (uppercase letter + underscore + number)
    m = re.search(r"#[ABab]_(\d+)$", href)
    if m:
        return m.group(1)
...
def render_inline(elem: ET.Element, footnotes: dict[str, str] | None = None) -> str:
    ...
    for child in list(elem):
        ...
        if ctag in {"strong", "b"}:
            ...
            href = child.get("href", "")
            a_class = child.get("class", "")
            if "duokan-footnote" in a_class:
                # footnote reference: skip the img inside, emit [^N] directly
                note_num = extract_footnote_ref_from_href(href)
                if note_num:
...
def render_block(
    ...
    if tag == "p":
        ...
        if klass == "title":
            if text:
                return [f"## {text}", ""]
            return []
        if klass == "chapter_number":
            return []
        if klass == "dialog_sub_or":
    ...
    if tag == "ul":
        if skip_footnote_blocks and "duokan-footnote-content" in elem.get("class", ""):
            return []
        ...
        for child in list(elem):
            ...
            if ctag == "img":
                ...
                if src:
                    ...
                    lines.append("")
            elif ctag == "figcaption":
                caption = render_inline(child, footnotes=footnotes)
                if caption:
    ...
    if tag == "table":
        return render_table(elem, footnotes=footnotes)
...
def render_table(elem: ET.Element, footnotes: dict[str, str] | None = None) -> list[str]:
    """Render an HTML table to Markdown pipe-table format."""
    # Collect all rows from thead/tbody/tr directly under table
    rows: list[list[tuple[str, int]]] = []  # each row: list of (cell_text, colspan)
    for child in elem.iter():
        ...
        for cell in list(child):
            ccell = strip_ns(cell.tag)
            if ccell not in {"td", "th"}:
                continue
            colspan = int(cell.get("colspan", "1"))
            text = render_inline(cell, footnotes=footnotes).strip()
    ...
    # Determine max columns (accounting for colspan)
    max_cols = max(sum(span for _, span in row) for row in rows)
...
def extract_footnotes(body: ET.Element) -> dict[str, str]:
    ...
    # duokan structure: <ol class="duokan-footnote-content"><li id="B_N">...
    for ol in body.iter(f"{{{HTML_NS}}}ol"):
        if "duokan-footnote-content" not in ol.get("class", ""):
            continue
        for li in ol.findall(f"{{{HTML_NS}}}li"):
            li_id = li.get("id", "")
            m = re.match(r"^[Bb]_(\d+)$", li_id)
            if not m:
                continue
            note_num = m.group(1)
            # Collect plain text from all descendant nodes, skip back-link anchors
            parts: list[str] = []
            for node in li.iter():
                ntag = strip_ns(node.tag)
                if ntag == "a" and re.search(r"#[Aa]_\d+$", node.get("href", "")):
                    # back-reference anchor: include text but skip as a link
                    if node.text:
                        parts.append(normalize_text(node.text))
                    continue
                if node.text and ntag not in {"li"}:
                    parts.append(normalize_text(node.text))
                if node.tail and strip_ns(node.tag) != "li":
                    parts.append(normalize_text(node.tail))
...
def copy_images_and_fix_paths(content: str, xhtml_path: Path, out_dir: Path) -> str:
    """Copy images referenced in markdown to out_dir/images/ and rewrite their paths."""
    ...
    def replace_img(m: re.Match) -> str:
        ...
        if src.startswith(("http://", "https://", "//", "/")):
            return m.group(0)
        img_path = (xhtml_path.parent / src).resolve()
...
def main() -> int:
    ...
    for idx, entry in enumerate(chapters, start=0):
        ...
        body_md = copy_images_and_fix_paths(body_md, xhtml_path, out_dir)
